...
public class Placanje extends JFrame {
	...
	private JButton btnOK = new JButton("Uplata");
	...
	public Placanje(Biblioteka biblioteka) {
	...
	private void initAction() {
		...
		btnOK.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String ClanId = String.valueOf(n.charAt(0));
				
				String brojMeseci = boxMeseci.getSelectedItem().toString();
				
				ClanBiblioteke clan = biblioteka.nadjiClana(ClanId);
				
				clan.setBrojMeseciClan(Integer.parseInt(brojMeseci));
				clan.setAktivan(true);
				clan.setDatumPoslUplate(LocalDate.now());
				biblioteka.snimiClanove();
				
				int uplaceniBrojMeseci = clan.getBrojMeseciClan();
				
				double cena = clan.getTip().getCena();
				
				String poruka;
				
				if(uplaceniBrojMeseci == 6) {
					
					cena = (cena * uplaceniBrojMeseci *90) / 100;
					...
					if(uplaceniBrojMeseci == 12) {
						...
						cena = (cena * uplaceniBrojMeseci*80) / 100;
